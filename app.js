...
let isShuffle = false;
let isRepeat = false;
let favorites = new Set();      // Spotify track IDs
let history = [];
let playlists = [];
...
// Admin uploaded songs (Firestore)
let adminSongs = [];
...
async function loadAdminSongs() {
  try {
    ...
    adminSongs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (e) {
    adminSongs = [];
  }
}
...
async function renderHome() {
  ...
  // Hero
  const heroTrack = trending[0] || afro[0];
  if (heroTrack) {
    heroSong = heroTrack;
  ...
  // New releases carousel — use album covers + first track on play
...
function createAlbumCard(album) {
  ...
  `;
  // On click search for the album's tracks and play the first one
  div.onclick = async () => {
...
function createRow(song, num, queue = [], opts = {}) {
  ...
  div.id = 'row-' + song.id;
  const liked = favorites.has(song.id || song.spotifyId);
  const hasPrev = !!song.preview_url;
  ...
  div.innerHTML = `
    ...
    <div class="row-info">
      ...
      <div class="row-artist">${esc(song.artist)}${hasPrev ? ' <span class="preview-pill">PREVIEW</span>' : ''}</div>
    </div>
    <div class="row-right">
      <button class="like-row-btn${liked ? ' liked' : ''}" onclick="event.stopPropagation();toggleLike('${song.id}',this)" aria-label="Like">
      ...
      ${opts.isAdmin ? `
        <button class="icon-btn more-row-btn" onclick="event.stopPropagation();adminDelete('${song.id}','${esc(song.audio_url||'')}','${esc(song.cover_url||'')}')">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
        </button>
      ` : `
        <button class="more-row-btn icon-btn" onclick="event.stopPropagation();openSheet(${JSON.stringify(song).replace(/"/g,'&quot;')})">
...
function playSong(song, queue = []) {
  ...
  if (hasPrev) {
    ...
    // No preview — show Spotify fallback in player, don't play audio
    showToast('No preview — open in Spotify');
  }

  updatePlayerUI(song);
  addToHistory(song);
  highlightRow(song.id);
...
function setPlayIcons(playing) {
  ...
  const mainPauseIcon = `<svg viewBox="0 0 24 24" fill="var(--bg0)"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>`;
  const mainPlayIcon = `<svg viewBox="0 0 24 24" fill="var(--bg0)"><polygon points="5 3 19 12 5 21 5 3"/></svg>`;
  document.getElementById('mini-play-btn').innerHTML = playing ? pauseIcon : playIcon;
  document.getElementById('main-play').innerHTML = playing ? mainPauseIcon : mainPlayIcon;
...
function updatePlayerUI(song) {
  ...
  if (!song.preview_url && song.spotify_url) {
...
function updateLikeUI() {
  ...
  [document.getElementById('full-like'), document.getElementById('mini-like')].forEach(b => {
    if (!b) return;
...
window.toggleLike = async (songId, btn) => {
  const song = currentQueue.find(s => s.id === songId) || currentSong;
  if (!song) return;
...
async function renderLikedSongs() {
  ...
  if (!isGuest) {
    ...
    // From local we only stored IDs — can't refetch without network so show empty
    songs = [];
  }
...
function renderHistoryPanel() {
  ...
  list.innerHTML = '';
  const songs = isGuest ? history : history.map(h => h.trackData || h).filter(Boolean);
  if (!songs.length) { empty.classList.remove('hidden'); return; }
  empty.classList.add('hidden');
...
window.openSheet = (song) => {
  if (typeof song === 'string') {
    try { song = JSON.parse(song); } catch { return; }
  }
...
window.sheetLike = () => {
  closeSheet();
  if (sheetTargetSong) toggleLike(sheetTargetSong.id, null);
};
window.sheetAddPlaylist = () => {
  closeSheet();
  if (sheetTargetSong) openPlPick(sheetTargetSong);
};
  ...
  try {
    ...
    await new Promise((res, rej) => {
      ...
      task.on('state_changed', snap => {
        fill.style.width = (snap.bytesTransferred / snap.totalBytes * 70) + '%';
      }, rej, async () => { audio_url = await getDownloadURL(task.snapshot.ref); res(); });
    });
    if (_coverFile) {
      ...
      await new Promise((res, rej) => cTask.on('state_changed', null, rej, async () => { cover_url = await getDownloadURL(cTask.snapshot.ref); res(); }));
    }
    ...
    adminSongs.unshift({ id: docRef.id, ...data });
    fill.style.width = '100%'; txt.textContent = '✅ Upload complete!';
    setTimeout(() => { prog.style.display = 'none'; fill.style.width = '0%'; document.getElementById('up-title').value = ''; document.getElementById('up-artist').value = ''; document.getElementById('up-genre').value = ''; document.getElementById('audio-label').textContent = 'Choose MP3 file *'; document.getElementById('cover-preview').innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg><span>Cover Art</span>`; _audioFile = null; _coverFile = null; }, 2000);
    renderAdminSongs();
...
function genreEmoji(song) {
  const map = { Afrobeats:'🎺', 'Hip-Hop':'🎤', 'R&B':'🎸', Pop:'🎵', Gospel:'🙏', Highlife:'🥁', Dancehall:'🏝️', Drill:'🎧', Amapiano:'🎹' };
